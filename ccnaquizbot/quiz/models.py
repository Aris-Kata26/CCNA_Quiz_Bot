...
class Question(models.Model):
    ...
    def get_original_image_url(self):
        ...
        # If it's a CloudinaryField object
...
class Answer(models.Model):
    ...
    def __str__(self):
        title = self.question.title[:30]
        ans = self.answer[:30]
        return f"{title}... - {ans}" if len(self.answer) > 30 else f"{title} - {ans}"

    def clean(self):
        """
        Custom validation to ensure only one correct answer exists per question.
        Only run this check if the related question already exists (has a PK).
        """
